...
- **`IRONBOUND.md`** (this file) — The engine. Handles loading, dev mode detection, memory protection, and integrity verification. Agent files (CLAUDE.md, GEMINI.md, etc.) do not exist in the repo — they are build artifacts generated by `scripts/build.js` into `dist/`.
