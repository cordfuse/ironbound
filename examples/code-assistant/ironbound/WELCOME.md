...
```bash
EXISTING_VERSION=$(defaults read ~/Desktop/Pairbot.app/Contents/Info.plist IronBoundVersion 2>/dev/null)
EXISTING_PATH=$(defaults read ~/Desktop/Pairbot.app/Contents/Info.plist IronBoundPath 2>/dev/null)
```

**Linux** — grep metadata from the .desktop file:
```bash
EXISTING_VERSION=$(grep '^X-IronBound-Version=' ~/Desktop/Pairbot.desktop 2>/dev/null | cut -d= -f2)
EXISTING_PATH=$(grep '^X-IronBound-Path=' ~/Desktop/Pairbot.desktop 2>/dev/null | cut -d= -f2)
```
