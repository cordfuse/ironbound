...
### No Agent Files in the Repo

Agent files (CLAUDE.md, GEMINI.md, AGENTS.md, .windsurfrules, .clinerules) do **not** exist in the repo during development. This means your IDE agent (Claude Code, Cursor, etc.) acts normally — no persona constraints while you're coding.

Agent files are **build artifacts** — generated by `scripts/build.js` into `dist/`. They only exist in production builds (the ZIP that ships to users).
...
`scripts/build.js` generates a complete production-ready build in `dist/`:

1. Strips dev mode from IRONBOUND.md
2. Generates SHA-256 checksum
3. Creates all agent files (CLAUDE.md, GEMINI.md, AGENTS.md, .windsurfrules, .clinerules) from the clean IRONBOUND.md
...
1. Runs `scripts/build.js` — strips dev mode, generates agent files and checksum into `dist/`
